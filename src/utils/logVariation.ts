...
export const logVariation = () => {
  const devcycleClient = getDevCycleCloudClient()

  let idx = 0
  const renderFrame = async () => {
    const features = await devcycleClient.allFeatures(SERVICE_USER)
    const { variationName = "Default" } = features["hello-togglebot"] ?? {}

    const wink = await devcycleClient.variableValue(
      SERVICE_USER,
      "togglebot-wink",
      false
    )
    const speed = await devcycleClient.variableValue(
      SERVICE_USER,
      "togglebot-speed",
      "off"
    )
